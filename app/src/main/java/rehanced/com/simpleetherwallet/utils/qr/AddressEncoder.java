...
public class AddressEncoder {
    ...
    public static AddressEncoder decodeERC(String s) throws IOException {
        if(! s.startsWith("ethereum:") && ! s.startsWith("ETHEREUM:")) throw new IOException("Invalid data format, see ERC-67 https://github.com/ethereum/EIPs/issues/67");
    ...
    public static AddressEncoder decodeICAP(String s) throws IOException {
        if(! s.startsWith("iban:XE") && ! s.startsWith("IBAN:XE")) throw new IOException("Invalid data format, see ICAP https://github.com/ethereum/wiki/wiki/ICAP:-Inter-exchange-Client-Address-Protocol");
